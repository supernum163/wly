...
thumbnailImage: https://i.postimg.cc/Fzn6FfCw/R.png
---
...
$$\begin{cases}
  Z_{ij} = X[i,] * Y[,j] &\text{if } (Z = X \\%*\\% Y)\\\\ 
  Z_{ij} = X[i,j] * Y[i,j] &\text{if } (Z = X * Y)
\end{cases}$$
...
# 行列式
对于$n \times n$阶矩阵**X**，我们可以使用{{< hl-text primary >}}det{{< /hl-text >}}函数来求解其**行列式**。注意矩阵的行列式可以写成以下形式：

$$det(X) = \sum (-1)^K * X_{1k_1} * X_{2k_2}*...*X_{nk_n}$$

<br>

# 特征值与特征向量

假设$Ax = \lambda x$，其中**A**为矩阵，**x**为向量，$\lambda$为数字。则$\lambda$、**x**分别为矩阵**A**的**特征值**与**特征向量**（注意矩阵的特征值与特征向量可能不止有一个）。

在R语言中我们可以通过{{< hl-text primary >}}eigen{{< /hl-text >}}函数，来求解任意$n \times n$阶矩阵的特征值与特征向量。
...
```
$$X %*% Y$vectors[,i] = Y$vectors[,i] * Y$values[i]$$
...
假设$$X = U %*% D %*% V^T$$，其中**X**为任意$n \times m$阶矩阵，**D**为**对角矩阵**，**U、V**均为由**单位向量**组成的方阵。则**D**中对角线上的元素为矩阵**X**的**奇异值**，**U、V**分别为矩阵**X**的**左、右奇异矩阵**。

在R语言中我们可以通过{{< hl-text primary >}}svd{{< /hl-text >}}函数，来对任意矩阵进行奇异值分解。
...
对于$n \times n$阶矩阵**A**，如果$A*A^T = E$，其中**E**为**单位矩阵**，则称**A**为**正交矩阵**。对于矩阵**A**，假设$A = Q %*% R$，其中**Q**为正交矩阵，**R**为上三角矩阵。
