...
定义S4分发函数及相应的S4方法的过程与S3非常类似。首先我们需要使用{{< hl-text primary >}}setGeneric(name, signature, def, group, valueClass, where, package, useAsDefault, simpleInheritanceOnly, ...){{< /hl-text >}}的形式定义一个S4分发函数，其中：

- **name**即分发函数的名称，必须与**setMethod**函数中的**f**参数一一对应
- **signature**即那些参数的数据类型需要验证，默认会验证全部参数的数据类型，不定长参数列表除外
- **def**即分发函数收到参数时应该如何处理，一般情况下我们需要直接使用{{< hl-text primary >}}standardGeneric{{< /hl-text >}}函数进行函数分发，当然我们也可以在这里处理接收到的参数或者方法的返回值
