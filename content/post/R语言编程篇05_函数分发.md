...
S3与S4系统有一个共同的特点——函数与数据是相是对分离的。为了将数据与函数结合（类似java中的类），**methods**程序包中还定义了一种RC类型的数据对象（**ReferenceClasses**）。RC对象实际上是由S4对象实现的，但RC对象的设计理念却与S4对象十分不同。
...
- 数据与函数结合的形式，与直接将数据传递给函数的形式有什么不同？
