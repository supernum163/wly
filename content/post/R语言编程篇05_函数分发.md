...
- **signature**即哪些参数的数据类型需要验证，默认会验证全部参数的数据类型，不定长参数列表除外。
- **valueClass**参数需要以字符串数组的形式指明返回值的有效数据类型，此时如果S4方法中返回了不同类型的值，则会触发异常。
...
- **group**参数，用于**分组分发函数**（**groupGeneric**）。

接下来我们需要使用{{< hl-text primary >}}setMethod(f, signature, definition, ...){{< /hl-text >}}的形式定义S4方法，其中的参数与**setGeneric**函数中的是互相对应的：**f**即我们之前定义的S4分发函数；**signature**中我们需要指明每个参数的数据类型；**definition**即方法的实现函数，注意方法的参数列表必须与分发函数中**def**定义的一致。

S4分发函数被调用时，会严格根据所有参数（不定长参数列表**...**除外）的数据类型分配S4方法，如果没有定义默认方法，分发函数找不到合适的方法时则会抛出异常。
