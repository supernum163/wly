...
thumbnailImage: https://n.png
---

**调用外部函数**。这篇文章我们将介绍R语言中如何调用外部函数。
...
使用**system、system2**函数，我们不仅可以运行操作系统命令（如**ls、cd**等），还可以调用其它计算机语言（如**R、python、Java、C/C++**等），但这种调用方式并不能直接将命令返回值转化为相应的R语言对象，而且直接将输出结果转化为字符串，往往会破坏输出结果的数据结构。所以这种方式更适合调用操作系统命令行，完成一些不需要用到返回值的操作，比如编译C/C++脚本、后台运行某项任务等。当然我们也可以借助字符串类型的数据交换格式，在操作系统命令行与R语言命令行之间传递数据。比如我们可以将输出结果转化为json字符串，然后在R语言中解析json字符串，即可将输出结果转化为相应的R语言对象。
...
上文我们已经介绍了如何在R语言中通过操作系统命令行调用其它计算机语言，但这种方法必须使用操作系统标注输出，才能将其它计算机语言的计算结果（返回值）传递给R语言。当然我们也可以使用第三方程序包（比如{{< hl-text purple >}}rPython、rJava{{< /hl-text >}}等程序包），直接在R语言中调用特定的计算机语言。但真正实现两种计算机语言之间，数据无缝对接的程序包很少，比如**rPython**依然是通过json字符串在R与pthon之间传递数据的。

以下代码展示了，如何使用**rJava**程序包，在R与Java之间进行数据传递、方法调用。关于更多**rava**的使用帮助，请参考 [这里](http://www.rforge.net/rJava/)。
...
关于更多python的C/C++接口规范，请参考 [这里](https://docs.python.org/zh-cn/2.7/c-api/)。
...
void python_exec(char** code, int* status, char** result, char** error)
{
  Py_Initialize();
  
  PyObject *pmod, *pdict, *pfunc, *pstr, *pres, *perr;
  pmod  = PyImport_ImportModule("json");
  pfunc = PyObject_GetAttrString(pmod, "json.dumps");
 
  pmod = PyImport_AddModule("__main__");
  pdict = PyModule_GetDict(pmod);
  
  pres = PyRun_String(*code, Py_eval_input, pdict, pdict); 
  perr = PyDict_GetItemString(pdict, "rPython_error" );
  
  if (perr != NULL){
    *status = -1;
    pstr = PyEval_CallObject(pfunc, perr);
    *error = PyString_AS_STRING(pstr);
    return;
  }
  ...
  pstr = PyEval_CallObject(pfunc, pres);
  *result = PyString_AS_STRING(pstr);
...
# 在R语言中调用python
.C("python_exec", "print 123", 0, "", "")

abc <- .C("python_exec", "123", 0, "", ""); abc

# 在R语言中编写包裹函数
python_exec <- function(code){
  code <- paste( "try:", paste( code, collapse = "\n" ), 
                 "except Exception as e:_r_error = e.__str__()\n", 
                 sep = "\n" )
  res <- .C("python_exec", code, status = 0, result = "", error = "")
  
  if(status = -1) stop(res$error)
  else if(status = 1) return(invisible(1))
