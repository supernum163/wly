...
$$ trunc(1.2345) = 1, \quad trunc(-1.2345) = -1, \quad signif(1.2345, 3) = 1.23, \quad zapsmall(1.2345, 3) = 1.235  $$
...
我们可以通过`factorial(x)`求`x`的阶乘，通过`choose(n, k)`的形式，求从`n`个样本中随机抽取`k`个会有多少种不同的组合。
