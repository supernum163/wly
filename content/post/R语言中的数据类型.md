...
我们已经知道，R语言中所有**vector**类型的对象，实际上都包含一个**vecsxp**结构体，而**vecsxp**包含两个整形变量**length、truelength**用以指示数组的长度，所以R语言中的数组长度是有上限的。在**R 3.0.0**之前的版本中，只有**length**一个变量被用于指示数组的长度，最多可以支持长度为`2^31`（大约21亿）的数组。而在之后的版本中，数组的最大长度可以为`2^52`。
